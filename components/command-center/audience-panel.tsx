...
export function AudiencePanel() {
  return (
    <div className="space-y-3">
      <div className="rounded-[3px] border border-[#333333] bg-[#111112] p-4">
        <div className="mb-1 flex items-center justify-between">
          <span className="text-[0.78rem] font-bold uppercase tracking-[0.14em] text-[#e86f3a]">Audience Overview</span>
          <span className="inline-flex items-center gap-[5px] text-[0.72rem] uppercase tracking-[0.12em] text-[#8d877e]">
            <span className="h-[5px] w-[5px] rounded-full bg-[#6f6a61]" /> mock
          </span>
        </div>
        <p className="text-[0.9rem] text-[#8d877e]">Set YOUTUBE_ANALYTICS_TOKEN to enable live cohort data.</p>
      </div>
          ...
          <div className="space-y-[7px]">
            {DEVICES.map((d) => (
              <div key={d.label} className="grid grid-cols-[70px_1fr_36px] items-center gap-2">
                <span className="text-[0.88rem] text-[#f4f1e8]">{d.label}</span>
                <div className="h-[5px] overflow-hidden rounded-[2px] bg-[#1a1c18]">
                  <div className="h-full rounded-[2px]" style={{ width: `${d.pct}%`, background: d.color }} />
                </div>
                <span className="text-right text-[0.78rem] text-[#8b857b]">{d.pct}%</span>
              </div>
            ))}
          </div>
        </Section>
      ...
      <div className="grid gap-3 md:grid-cols-2">
          ...
          <div className="space-y-[7px]">
            {TRAFFIC.map((t) => (
              <div key={t.label} className="grid grid-cols-[120px_1fr_36px] items-center gap-2">
                <span className="text-[0.88rem] text-[#f4f1e8]">{t.label}</span>
                <div className="h-[5px] overflow-hidden rounded-[2px] bg-[#1a1c18]">
                  <div className="h-full rounded-[2px] bg-[#5cb8e0]/70" style={{ width: `${t.pct}%` }} />
                </div>
                <span className="text-right text-[0.78rem] text-[#8b857b]">{t.pct}%</span>
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section title="Retention Curve">
          <div className="mb-2 text-[0.84rem] text-[#8b857b]">
          ...
          <div className="mt-2 flex justify-between text-[0.68rem] uppercase tracking-[0.1em] text-[#6f6a61]">
            <span>0%</span>
            <span>50%</span>
            <span>100%</span>
          </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-[3px] border border-[#333333] bg-[#111112] p-4">
      <div className="mb-4 text-[0.78rem] font-bold uppercase tracking-[0.14em] text-[#e86f3a]">{title}</div>
      {children}
    </div>
  );
}

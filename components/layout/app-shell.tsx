...
export function AppShell({ children, wide = false }: { children: React.ReactNode; wide?: boolean }) {
  ...
  const headerPadding = wide ? "px-2 sm:px-3 md:px-4" : "px-3 sm:px-5 md:px-8";
  const mainPadding = wide ? "px-0 pb-10" : "px-3 pb-10 sm:px-5 md:px-8";
      ...
      <header className="pb-3 pt-6 sm:pt-10 md:pt-12">
        <div className={`${frameClass} ${headerPadding}`}>
          <div className="flex flex-wrap items-start justify-between gap-3 sm:gap-6">
            ...
            <div className="hidden pt-3 text-[0.72rem] uppercase tracking-[0.16em] text-[#8d877e] lg:block">
          ...
          <div className="mt-4 flex flex-col gap-3 sm:mt-7 md:flex-row md:items-start md:justify-between md:gap-4">
            <nav className="-mx-1 flex flex-nowrap items-center gap-2 overflow-x-auto px-1 thin-scrollbar md:flex-1 md:flex-wrap md:overflow-visible">
              {nav.map(([label, href, Icon]) => (
                <Link
                  key={label}
                  href={href}
                  className="inline-flex h-9 shrink-0 items-center gap-2 rounded-[3px] border border-[#333333] bg-[#151515] px-4 text-[0.75rem] font-bold uppercase tracking-[0.1em] text-[#f6efe5] shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_0_0_1px_rgba(0,0,0,0.55)] transition hover:border-[#333333] hover:bg-[#1d1715] hover:text-[#e97848]"
                >
            ...
            <div className="flex h-9 shrink-0 items-center gap-2 self-start">
              <ThemeToggle />
              <Badge tone="green" className="h-9 gap-2 px-3 text-[0.75rem]">
